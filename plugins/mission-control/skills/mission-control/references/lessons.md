...
Each rule in the skill exists because a real session paid for it. Lessons 1–16: distilled from the omniforge program, 2026-09-06 → 09-15 (manual, pre-skill practice — the discipline the skill transcribes). Lessons and annotations revised from the det-filter program, 2026-09-15 → 09-17 — the skill's own first run. Until v1.3.0 the shipped artifact itself had never been executed.

1. **The stale-base trap.** A repo's main checkout sat *behind the deployed engine*; sessions forking from "whatever is checked out" would have silently invalidated themselves. → Base verification immediately after kickoff with the tiered mismatch policy (descends-from + owned-file-delta → adopt loudly; non-ancestor or owned-file intersection → STOP; never silently), per-repo base rules in the facts note, and the launch-time recheck line in every launch block.
...
3. **Unattended sessions stall on questions.** Unanswered unattended Clarify questions adopt recommended defaults as recorded assumptions; the deep plan gate is pre-approved when autonomy=unattended and otherwise bounded (1800 s headless, then denial). An active goal owned by another program in the same repo is a continuation-misroute risk (auto-invoke reuses its slug for plausible continuations), not a stall. → Prompts pre-answer the Clarify batch and record it as assumptions — autonomy=unattended is the load-bearing pre-answer; every lane invocation passes `--slug`, and verify checks goal provenance after kickoff — a routing check, never a kickoff STOP.
...
6. **regenloop-run needs an onboarded repo.** A repo without `gates.toml`/orchestrator state is a clean hard stop at the doctor preflight, before any dispatch (and `--require-evidence` makes a zero-gate green `no_evidence`). → Lanes in non-onboarded repos run as plain deep sessions with the discipline written into the prompt; onboarding is tracked as its own work item.
